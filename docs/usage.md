# nf-core/egatransfer: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/egatransfer/usage](https://nf-co.re/egatransfer/usage)
...
```console
nextflow run nf-core/egatransfer --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/egatransfer
```
...
First, go to the [nf-core/egatransfer releases page](https://github.com/nf-core/egatransfer/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
